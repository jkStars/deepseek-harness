...
- **Plugin-set changes reload the page** — a graph-rev change (a live plugin add/remove) cannot hot-swap rows the boot manifest never named, so the browser reloads instead; the page reload is the intended granularity for plugin-set changes.
- **Graph rev is not refreshed by rebuilt frames** — the stale rev is harmless because the bundle endpoint serves no-cache; only reconnect or a graph change refreshes it.
