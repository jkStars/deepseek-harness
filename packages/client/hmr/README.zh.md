...
浏览器侧订阅系统 SSE（Server-Sent Events）通道（`GET /plugins/events`），每个 `rebuilt` 帧重载一个插件，并通过队列串行执行。每帧的顺序是：`invalidate`、`prefetch`（旧 fiber 仍在服务时加载并注册新组合包）、`registry.delete`（在 fiber dispose（资源释放）之前执行：仅 dispose fiber 会触发 vendored Loader 的 self-dispose 分支，把配置项标为禁用）、排空旧 fiber、删除 `entry.fiber`、移除自身拥有的 `<style data-plugin>` 标签、通过 `entry.refresh()` 重新导入并挂载、通过 `fiber.await()` 直接重新抛出启动失败。依赖方由 Cordis 自身重载：fiber 的激活 epoch 会串联其服务提供方的 uid，因此替换提供方 fiber 会级联所有依赖方，无需客户端图分析。node 侧使用一个 interval 检测重建：从同步基线开始 stat-poll 每个图组合包；新增一行后立即重新计算 hash；缺失行保持 dirty；只广播真实 rev 变更。因此，任何生成组合包的 tsdown watch 进程都能触发 HMR（热模块替换），无需 builder→host 通道。node 侧还会广播每次图（graph）变更（不仅是连接时的快照）：`dsh plugin add/remove` 热挂载或卸载页面启动清单中从未出现过的客户端行，浏览器侧在收到 rev 与页面启动 rev 不一致的 graph 帧时执行一次 `location.reload()`——新插件 UI 无需手动刷新即可出现，且由于新页面携带的 manifest 带有新 rev，重载不会循环。
...
- **插件集合变更会整页重载**：图 rev 变更（live plugin add/remove）无法热换启动清单中从未命名过的行，因此浏览器选择整页重载；对插件集合变更来说，整页重载正是预期的粒度。
- **重建帧不会刷新图 rev**：陈旧 rev 无害，因为组合包端点以 no-cache 提供内容；只有重新连接或图变更时才会刷新。
